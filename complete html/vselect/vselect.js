...
$.fn.vSelect = function(s) {
  ...
  function init(sElm, s) {
    ...
    const vSelectTray = $('<div class="vselect-tray '+(settings.dropdown ? 'dropdown' : '')+'" id="vselect-tray'+randomId+'" style="max-height: '+settings.trayHeight+'"></div>');
    // Append options tray
    ...
    function updateDisplay() {
      ...
      if (checkedOptions.length > 0) {
        if (settings.display === 'sum') {
          vSelectElm.find('.vselect-display-text').text(settings.selectedLabel+':' + checkedOptions.length);
