...
add_action( 'wp_trash_post', 'get_deleted_booking_id' );

function get_deleted_booking_id( $post_id ) {
    // Get the post object
    ...
    if ( $post->post_type === 'bookings' ) {
        ...
        $selectedDateID = get_post_meta( $post_id, 'selectedDateID', true );
    //      print_r($selectedDateID);
    // wp_die();
        // If selectedDateID is found
        if ( $selectedDateID ) {
            // Delete the meta value with the specified keys
            delete_post_meta( $selectedDateID, '_is_booking_booked' );
            delete_post_meta( $selectedDateID, '_booking_status' );
        }
    }
}
...
function submit_booking_form() {
    ...
    $termin_konsultacj      = date("d.m.Y", strtotime($termin_konsultacj));
    ...
    $booking_timestamp = strtotime($termin_konsultacj . " " . $godzina_konsultacji);
        // date_default_timezone_set('Europe/Warsaw');
        $poland_time = new DateTime('now', new DateTimeZone('Europe/Warsaw'));
        $current_poland_timestamp = $poland_time->getTimestamp(); // Current timestamp
        // $newtime = $booking_timestamp - 7200; // Adjusting by 2 hours
        $newtimepoland=  $current_poland_timestamp + 7200;
        // Convert booking timestamp to DateTime object for formatting
        $booking_time = (new DateTime())->setTimestamp($booking_timestamp);
        $current_poland_time = (new DateTime())->setTimestamp($newtimepoland); // Convert current timestamp
        // echo $newtimepoland . ' | ' . $booking_timestamp;
        // Check if the booking timestamp is in the past
       
        if ($booking_timestamp < $newtimepoland) {
            // Stop the form from being submitted if booking time is in the past
            wp_die('error');
        }
    // $formatted_time = date('g:i A n/j/Y', $poland_time);
    //  print_r($category_name);
    //  wp_die();
    $booking_post = array(
        'post_title'        => 'Zapis na konsultacje: ' . $name . ' ' . $nazwisko,
        'post_type'         => 'bookings',
        'post_status'       => 'publish',
    );
    ...
    update_post_meta($selectedDate, '_is_booking_booked', true);
    update_post_meta($selectedDate, '_booking_status', 'Zarezerwowane');
    update_post_meta($post_id, 'selectedDateID',  $selectedDate);
    ...
    update_post_meta($post_id, 'termin_konsultacji', $termin_konsultacj);
    ...
    $notification_time =  $booking_timestamp + 3600; // Add 1 hour in seconds
    // $notification_time = strtotime($notification_time);
    // Schedule a single event to send email
    wp_schedule_single_event($notification_time, 'send_email_notification', array($post_id ));
    $title          =   '';
    $content        =   '';
    $admincontent   =   '';
    $admintitle     =   '';
    if($form_consultation == 'w biurze PdP'){
        ...
        $content    .=  '<p style="-ms-text-size-adjust:100%;-webkit-text-size-adjust:100%;margin-top:0;margin-bottom:15px;line-height:1.6;">
                            ...
                            dziękujemy za zapisanie się na konsultacje w biurze PdP.
                        </p>';
        $content    .= '<p>Termin i godzina konsultacji: '.$termin_konsultacj.' '.$godzina_konsultacji.'</p>';
       
        $content    .= '<p>Pracownicy biura PdP skontaktują się z Państwem w celu ustalenia szczegółów konsultacji.<br>
                        Do usłyszenia!<br>
                        Zespół Punktu dla Przyrody<br>
                        </p>';

        $admintitle      = ' Zapis na konsultacje w biurze PdP';
        $admincontent    = '';
        $admincontent    .=  '<p style="-ms-text-size-adjust:100%;-webkit-text-size-adjust:100%;margin-top:0;margin-bottom:15px;line-height:1.6;">
                           W systemie Punktu dla Przyrody zostało odebrane zgłoszenie na konsultacje w biurze PdP. 
                        </p>';
        $admincontent    .= '<p>Imię i nazwisko: '.$name.' '.$nazwisko.'<br>
                          ...
                          Temat/zakres konsultacji: '.$topic.' <br>
                           Termin i godzina konsultacji:  '.$termin_konsultacj.' '.$godzina_konsultacji.' <br>
                           Rodzaj konsultacji: '.$form_consultation.' <br>
        ...
        $content    .=  '<p style="-ms-text-size-adjust:100%;-webkit-text-size-adjust:100%;margin-top:0;margin-bottom:15px;line-height:1.6;">
                            Szanowny Użytkowniku, <br>
                            dziękujemy za zapisanie się na konsultacje telefoniczne.
                        </p>';
        $content    .= '<p>Termin i godzina konsultacji: '.$termin_konsultacj.' '.$godzina_konsultacji.' <br>
        ...
        $admincontent    .= '<p>Imię i nazwisko: '.$name.' '.$nazwisko.'<br>
                       ...
                       Temat/zakres konsultacji: '.$topic.' <br>
                        ...
                        Dziedzina: '.$categories_name.' <br>
                        informacje inne :   '.$informacje_inne.' <br>
        ...
        $content    .=  '<p style="-ms-text-size-adjust:100%;-webkit-text-size-adjust:100%;margin-top:0;margin-bottom:15px;line-height:1.6;">
                            Szanowny Użytkowniku, <br>
                            dziękujemy za zapisanie się na konsultacje e-mail.
                        </p>';
        $content    .= '<p>
                        Termin i godzina konsultacji:  '.$termin_konsultacj.' '.$godzina_konsultacji.' <br>
        ...
        $content    .= '<p>Nasz ekspert skontaktuje się z Państwem w wybranym terminie. <br>
                          Do usłyszenia!<br>
                          Zespół Punktu dla Przyrody
                        </p>';
                        

        $admintitle      = 'Zapis na konsultacje e-mail';
        $admincontent    = '';
        $admincontent    .=  '<p style="-ms-text-size-adjust:100%;-webkit-text-size-adjust:100%;margin-top:0;margin-bottom:15px;line-height:1.6;">
                             W systemie Punktu dla Przyrody zostało odebrane zgłoszenie na konsultacje e-mail.
                        </p>';
        $admincontent    .= '<p>Imię i nazwisko: '.$name.' '.$nazwisko.'<br>
                          E-mail uczestnika: '.$email.' <br>
                          Telefon uczestnika: '.$telefon.' <br>
        ...
        $content    .=  '<p style="-ms-text-size-adjust:100%;-webkit-text-size-adjust:100%;margin-top:0;margin-bottom:15px;line-height:1.6;">
                            Szanowny Użytkowniku, <br>
                            dziękujemy za zapisanie się na konsultacje online.
                        </p>';
        $content    .= '<p>
                            Termin i godzina konsultacji: '.$termin_konsultacj.' '.$godzina_konsultacji.' <br>
                            Dziedzina: '.$categories_name.'<br>
        ...
        $admincontent    = '';
        $admincontent    .=  '<p style="-ms-text-size-adjust:100%;-webkit-text-size-adjust:100%;margin-top:0;margin-bottom:15px;line-height:1.6;">
        W systemie Punktu dla Przyrody zostało odebrane zgłoszenie na konsultacje online. 
                        </p>';
        $admincontent    .= '<p>Imię i nazwisko: '.$name.' '.$nazwisko.'<br>
                            E-mail uczestnika: '.$email.' <br>
                            Telefon uczestnika: '.$telefon.' <br>
    ...
    if($form_consultation){
        ...
        $custom_email = get_option('submit_booking_form');
        $admin_email = get_option('admin_email');
        $w_biurze_PdP = get_option('_cwb_audyt_admin_email');          
        if ( $form_consultation =='w biurze PdP') {
            wp_mail($w_biurze_PdP, $admintitle, $admin_email_template , $headers_admin);
        }
    ...
    // echo $current_poland_time->format('Y-m-d H:i:s') . ' | ' . $booking_time->format('Y-m-d H:i:s');
    wp_die();
}
...
function send_email_notification($booking_id) {
   

   
           
            $emailtitle = get_the_title($booking_id);
           $email =  get_post_meta($booking_id, 'email', true);
                    update_post_meta($booking_id, 'email_already_sent_user', 'email is already sent user');

                    // Email content construction
                    $title = 'Dziękujemy za udział w konsultacjach PdP!' ;
                    $content = '<p style="-ms-text-size-adjust:100%;-webkit-text-size-adjust:100%;margin-top:0;margin-bottom:15px;line-height:1.6;">
                    Dziękujemy za skorzystanie z konsultacji Punktu dla Przyrody. 
                    </p>';
                    $content .= '<p style="-ms-text-size-adjust:100%;-webkit-text-size-adjust:100%;margin-top:0;margin-bottom:15px;line-height:1.6;">                   
                    Prosilibyśmy o wypełnienie ankiety znajdującej się pod linkiem: <a href="https://punktdlaprzyrody.lasy.gov.pl/ocena-konsultacji/">Ocena konsultacji</a> <br> Hasło do ankiety: konsultacje_pdp <br>
                    Pozdrawiamy,<br>
...
function submit_booking_form_second() {
    ...
    if (!empty($_FILES['filedatat']['name'])) {
        // Specify the directory where you want to save the uploaded file
        $upload_dir = wp_upload_dir();
        $uploadDirectory = $upload_dir['path'] . '/';

        // Move the uploaded file to the desired directory
        $fileTmpPath = $_FILES['filedatat']['tmp_name'];
        $fileName = $_FILES['filedatat']['name'];
        if (move_uploaded_file($fileTmpPath, $uploadDirectory . $fileName)) {
            // File upload successful
            $filedatapath = $uploadDirectory . $fileName; // Path to the uploaded file
            $filedata = $upload_dir['url'] . '/' . $fileName;
        } else {
            // File upload failed
            echo "Error uploading file: " . $_FILES['filedatat']['error'];
            return;
        }
    } else {
		  $filedata = '';
    ...
    // Insert post
    $booking_post = array(
        'post_title' => 'Audyt: ' . $name . ' ' . $nazwisko,
        'post_type' => 'audyt',
        'post_status' => 'pending',
    );
    $post_id = wp_insert_post($booking_post);
    ...
    update_post_meta($post_id, 'topic', $topic);
    update_post_meta($post_id, 'filedata',  $filedata);
    ...
    $admintitle     =   '';
    // Email content
    if($topic=='ocena/analiza wniosku o dofinansowanie wraz z załącznikami'){

        $title = 'Dziękujemy za zapisanie się na wykonanie oceny/analizy wniosku o dofinansowanie wraz z załącznikami!';
        
        $content .= '<p>Szanowny Użytkowniku,<br> dziękujemy za zapisanie się na wykonanie oceny/analizy wniosku o dofinansowanie wraz z załącznikami.<br></p>
                    <p>Termin: '.$termin_konsultacji .'<br>
                    Temat/zakres wsparcia: Ocena/analiza wniosku o dofinansowanie wraz z załącznikami <br>
                    Inne informacje: '.$informacje_inne .'<br> </p>
                   <p> Nasz ekspert skontaktuje się z Państwem w wybranym terminie.<br>
                     Do usłyszenia!  <br>
                     Zespół Punktu dla Przyrody <br>
        </p>';
       

        $admintitle = 'Zapis na audyt ocenę/analizę wniosku o dofinansowanie';
        $admincontent = '<p>Szanowny Użytkowniku,<br>W systemie Punkt dla Przyrody pojawiło się nowe zamówienie na wykonanie oceny/analizy wniosku o dofinansowanie wraz z załącznikami:</p>';
        $admincontent .= '<p>Imię: ' . $name . '<br>Nazwisko: ' . $nazwisko . '<br>Adres e-mail: ' . $email . '<br>Telefon: ' . $telefon . '<br>Nazwa wnioskodawcy lub beneficjenta: ' . $wnioskodawcy . '<br>Temat/zakres wsparcia: Ocena/analiza wniosku o dofinansowanie wraz z załącznikami<br>Termin: ' . $termin_konsultacji . '<br>Inne informacje: ' . $informacje_inne . '</p>';
        $admincontent .= '<p>W przypadku pytań skontaktuj się ze zgłoszoną osobą. </p>';

    }
    else if($topic=='audyt przedrealizacyjny'){

        $title = 'Dziękujemy za zapisanie się na audyt!';
       
        $content .= '<p>Szanowny Użytkowniku,<br>
        dziękujemy za zapisanie się na audyt przedrealizacyjny.<br></p>
                      <p>Termin: '.$termin_konsultacji .'<br>
                      Temat/zakres wsparcia: Audyt przedrealizacyjny <br>
                    Inne informacje: '.$informacje_inne .'<br> </p>
                   <p> Nasz ekspert skontaktuje się z Państwem w wybranym terminie. <br>
                     Do usłyszenia!  <br>
                     Zespół Punktu dla Przyrody 
        ...
        $admintitle = 'Zapis na audyt przedrealizacyjny';
        $admincontent = '<p>Szanowny Użytkowniku,<br>W systemie Punkt dla Przyrody pojawiło się nowe zamówienie audytu przedrealizacyjnego:<br></p>';
        $admincontent .= '<p>Imię: ' . $name . '<br>Nazwisko: ' . $nazwisko . '<br>Adres e-mail: ' . $email . '<br>Telefon: ' . $telefon . '<br>Nazwa wnioskodawcy lub beneficjenta:  ' . $wnioskodawcy . '<br> Temat/zakres wsparcia: Audyt przedrealizacyjny <br>Termin audytu: ' . $termin_konsultacji . '<br>Inne informacje: ' . $informacje_inne . '</p>';
        $admincontent .= '<p>W przypadku pytań skontaktuj się ze zgłoszoną osobą.  </p>';

    }
    else if($topic=='audyt w trakcie realizacji projektu'){
      
        $title = 'Dziękujemy za zapisanie się na audyt!';
        
        $content .= '<p>Szanowny Użytkowniku,<br>dziękujemy za zapisanie się na audyt w trakcie realizacji projektu.<br> </p>
                     <p> Termin: '.$termin_konsultacji .'<br>
                      Temat/zakres wsparcia: Audyt w trakcie realizacji projektu <br>
                    Inne informacje: '.$informacje_inne .'<br> </p>
                   <p> Nasz ekspert skontaktuje się z Państwem w wybranym terminie.<br>
                    Do usłyszenia!  <br>
                    Zespół Punktu dla Przyrody <br>
        ...
        $admintitle = 'Zapis na audyt w trakcie realizacji projektu';
        $admincontent = '<p>Szanowny Użytkowniku,<br>W systemie Punkt dla Przyrody pojawiło się nowe zamówienie audytu w takcie realizacji projektu:<br></p>';
        $admincontent .= '<p>Imię: ' . $name . '<br>Nazwisko: ' . $nazwisko . '<br>Adres e-mail: ' . $email . '<br>Telefon: ' . $telefon . '<br>Nazwa wnioskodawcy lub beneficjenta:   ' . $wnioskodawcy . '<br> Temat/zakres wsparcia: Audyt w trakcie realizacji projektu <br>Termin: ' . $termin_konsultacji . '<br>Inne informacje: ' . $informacje_inne . '</p>';
        $admincontent .= '<p>W przypadku pytań skontaktuj się ze zgłoszoną osobą.  </p>';
    }
    else if($topic=='audyt na zakończenie realizacji projektu'){
      
        $title = 'Dziękujemy za zapisanie się na audyt!';
       
        $content .= '<p>Szanowny Użytkowniku,<br>
        dziękujemy za zapisanie się na audyt na zakończenie realizacji projektu.<br></p>
                     <p> Termin: '.$termin_konsultacji .'<br>
                      Temat/zakres wsparcia: Audyt na zakończenie realizacji projektu <br>
                    Inne informacje: '.$informacje_inne .'<br></p>
                   <p> Nasz ekspert skontaktuje się z Państwem w wybranym terminie.<br>
                     Do usłyszenia!  <br>
                     Zespół Punktu dla Przyrody <br>
        ...
        $admintitle = 'Zapis na audyt na zakończenie realizacji projektu';
        $admincontent = '<p>Szanowny Użytkowniku,<br>W systemie Punkt dla Przyrody pojawiło się nowe zamówienie audytu w takcie realizacji projektu:<br></p>';
        $admincontent .= '<p>Imię: ' . $name . '<br>Nazwisko: ' . $nazwisko . '<br>Adres e-mail: ' . $email . '<br>Telefon: ' . $telefon . '<br>Nazwa wnioskodawcy lub beneficjenta:   ' . $wnioskodawcy . '<br> Temat/zakres wsparcia: Audyt na zakończenie realizacji projektu <br>Termin: ' . $termin_konsultacji . '<br>Inne informacje: ' . $informacje_inne . '</p>';
        $admincontent .= '<p>W przypadku pytań skontaktuj się ze zgłoszoną osobą.  </p>';
    }
    else if($topic=='wizyta doradcza w miejscu realizacji projektu/ w siedzibie beneficjenta'){
      
        $title = 'Dziękujemy za zapisanie się na wizytę doradczą!';
        
        $content .= '<p>Szanowny Użytkowniku,<br>
        dziękujemy za zapisanie się na wizytę doradczą w miejscu realizacji projektu/siedzibie beneficjenta.<br></p>
                    <p>  Termin: '.$termin_konsultacji .'<br>
                      Temat/zakres wsparcia: Wizyta doradcza w miejscu realizacji projektu/siedzibie beneficjenta <br>
                    Inne informacje: '.$informacje_inne .'<br> </p>
                   <p> Nasz ekspert skontaktuje się z Państwem w celu omówienia szczegółów wizyty.<br>
                     Do usłyszenia!  <br>
                     Zespół Punktu dla Przyrody <br>
        </p>';
       

        $admintitle = 'Zapis na wizytę doradczą w miejscu realizacji projektu/siedzibie beneficjenta';
        $admincontent = '<p>Szanowny Użytkowniku,<br>W systemie Punkt dla Przyrody pojawiło się nowe zamówienie – zapis na wizytę doradczą w miejscu realizacji projektu/siedzibie beneficjenta:<br></p>';
        $admincontent .= '<p>Imię: ' . $name . '<br>Nazwisko: ' . $nazwisko . '<br>Adres e-mail: ' . $email . '<br>Telefon: ' . $telefon . '<br>Nazwa wnioskodawcy lub beneficjenta:   ' . $wnioskodawcy . '<br> Temat/zakres wsparcia: Wizyta doradcza w miejscu realizacji projektu/siedzibie beneficjenta <br>Termin: ' . $termin_konsultacji . '<br>Inne informacje: ' . $informacje_inne . '</p>';
        $admincontent .= '<p>W przypadku pytań skontaktuj się ze zgłoszoną osobą.  </p>';
    }
    else if($topic=='ocena pomysłu na projekt'){
      
        $title = 'Dziękujemy za zapisanie się na ocenę pomysłu na projekt!';
        
        $content .= '<p>Szanowny Użytkowniku,<br>dziękujemy za zapisanie się na wykonanie oceny pomysłu na projekt.<br></p>
                      <p>Termin: '.$termin_konsultacji .'<br>
                      Temat/zakres wsparcia: Ocena pomysłu na projekt <br>
                    Inne informacje: '.$informacje_inne .'<br></p>
                  <p>  Nasz ekspert skontaktuje się z Państwem w wybranym terminie. <br>
                     Do usłyszenia!  <br>
                     Zespół Punktu dla Przyrody <br>
        ...
        $admintitle = 'Zapis na ocenę pomysłu na projekt';
        $admincontent = '<p>Szanowny Użytkowniku,<br>W systemie Punkt dla Przyrody pojawiło się nowe zamówienie – zapis na ocenę pomysłu na projekt:<br></p>';
        $admincontent .= '<p>Imię: ' . $name . '<br>Nazwisko: ' . $nazwisko . '<br>Adres e-mail: ' . $email . '<br>Telefon: ' . $telefon . '<br>Nazwa wnioskodawcy lub beneficjenta:   ' . $wnioskodawcy . '<br> Temat/zakres wsparcia: Ocena pomysłu na projekt <br>Termin: ' . $termin_konsultacji . '<br>Inne informacje: ' . $informacje_inne . '</p>';
        $admincontent .= '<p>W przypadku pytań skontaktuj się ze zgłoszoną osobą.  </p>';
    }
    ...
    if ($email) {
        ...
        $custom_email = get_option('my_custom_email-new');
        $admin_email = get_option('admin_email');
        

        // Add file as an attachment
        $attachments = array($filedatapath);
        ...
        // Send admin email
        if ($custom_email) {
            wp_mail($custom_email, $admintitle, $admin_email_template, $headers_admin, $attachments);
        } else {
            wp_mail($admin_email, $admintitle, $admin_email_template, $headers_admin, $attachments);
        }
    }
...
function create_new_booking_for_event() {
    ...
    if($totalseats <= $published_bookings){
        ...
        $current_date = current_time('Y-m-d H:i:s');  // Gets the current date and time in the correct format
        update_post_meta($post_id, 'booking_date', $current_date); // Save current date
        ...
        update_post_meta($post_id, 'booking_date', $current_date); // Save current date
        $current_date = current_time('Y-m-d H:i:s');  // Gets the current date and time in the correct format
        update_post_meta($post_id, 'booking_date', $current_date); 
...
function create_new_entry_for_szkolenia() {
    if (isset($_POST['emails'], $_POST['szkolen'], $_POST['post_type'])) {
        ...
        if (!empty($existing_post_id)) {
            echo 'email is repeated';
        } else {
            $postarr = array(
                'post_title' => $szkolen,
                'post_content' => '',
                'post_status' => 'publish', // Initial status is draft
                'post_type' => 'subscriber',
            );
            $post_id = wp_insert_post($postarr);
            ...
            if ($post_id) {
                ...
                // Prepare email content
                $title = 'Powiadomienia o szkoleniach';
                $content = '<p style="-ms-text-size-adjust:100%;-webkit-text-size-adjust:100%;margin-top:0;margin-bottom:15px;line-height:1.6;">
                    Dziękujemy za zapisanie się na powiadomienia o szkoleniach.<br>

                    </p>';
                $content .= '<p>Pozdrawiamy, <br>Pracownicy Punktu dla Przyrody</p>';
...
function confirm_subscriptions() {
    if (isset($_GET['token'])) {
        $confirmation_token = sanitize_text_field($_GET['token']);

        // Find the post with the matching confirmation token
        ...
        if ($query->have_posts()) {
            ...
            // Optionally, delete the confirmation token to prevent reuse
            delete_post_meta($post_id, 'confirmation_token');
...
add_action('init', 'confirm_subscriptions');
...
function create_new_post_for_qa() {
    ...
    if ($post_id) {
        ...
        $content .= '<p style="-ms-text-size-adjust:100%;-webkit-text-size-adjust:100%;margin-top:0;margin-bottom:15px;line-height:1.6;">
             Szanowny Użytkowniku,<br>
             W systemie Punkt dla Przyrody pojawiło się nowe zamówienie audytu. Zaloguj się do panelu w celu weryfikacji..
            </p>';
...
function get_dates_for_category_callback() {
   ...
   $defaulttimezone = date_default_timezone_get();
    ...
    $output = '<option value="" data-currenttime="'.$current_time.'">'.__("Termin konsultacji", "cwb").'</option>';
    ...
    if ($query->have_posts()) {
        while ($query->have_posts()) {
            ...
            $godzina_startu = get_post_meta(get_the_ID(), 'godzina_startu', true);
            if($godzina_startu <= $current_time && $date == $date_for_comparison){
                ...
                if (!in_array($formattedDate, $addedDates)) {
                    $output .= '<option value="'.esc_attr($date).'" data-postid="'.get_the_ID().'" data-currenttime="'.$defaulttimezone.'" data-currenttime="'.$current_time.'">'.esc_html($formattedDate).'</option>';
    ...
    $query = new WP_Query($args);
    $experts_options            =   '<option selected="" value="">'.__("Wybierz eksperta" , "cwb").'</option>';
    if ($query->have_posts()) {
        while ($query->have_posts()) {
                $query->the_post();
                $experts_options .= '<option value="'.get_the_ID().'">'.get_the_title().'</option>';
            }
        }
    wp_reset_postdata();
    $return_array['experts']    = $experts_options;
...
function get_times_for_date_callback() {
    ...
    if ($query->have_posts()) {
        while ($query->have_posts()) {
            $query->the_post();
            $bookingbooked = get_post_meta(get_the_ID(), '_is_booking_booked', true);
            $time = get_post_meta(get_the_ID(), 'godzina_startu', true);
            if($bookingbooked){
                $output .= '<option value="'.esc_attr($time).'" data-postidtest ="'.$bookingbooked.'"  data-postid="'.get_the_ID().'" disabled>'.esc_html($time).'</option>';
            }else if($date == $currentdae){
                if($current_time > $time){
                    $output .= '<option value="'.esc_attr($time).'" testcurrent="'.$currentdae.'" testdate="'.$date.'" data-postidtest ="'.$bookingbooked.'"  data-postid="'.get_the_ID().'"disabled>'.esc_html($time).'</option>';  
                }else{
                    $output .= '<option value="'.esc_attr($time).'" data-postidtest ="'.$bookingbooked.'"  data-postid="'.get_the_ID().'">'.esc_html($time).'</option>';   
                }
                
            }
            else{
                $output .= '<option value="'.esc_attr($time).'" testcurrent="'.$currentdae.'" testdate="'.$date.'" data-postidtest ="'.$bookingbooked.'"  data-postid="'.get_the_ID().'">'.esc_html($time).'</option>';  
            }
...
function list_slug_from_selected_news() {
    $selectedProgram = isset($_POST['programslug']) ? $_POST['programslug'] : array();
    $options = array();
    $categories = get_terms('un_email_groups', 'orderby=count&hide_empty=0');
    $programs = array();
    
    foreach ($categories as $category) {
        list($program, $dzial) = explode('+', $category->name, 2);
        $programs[$program][] = $dzial;
    }

    // Query for posts based on category slugs
    $args = array(); // Define your WP_Query arguments here

    $query = new WP_Query($args);
$addedOptions = array();
    // Loop through retrieved posts and add them as options
//     $dzialSelect = '<option value="">'.__('Wybierz' , 'nfinity-newsletter').'</option>';
 foreach ($selectedProgram as $slugloop) {
    if (isset($programs[$slugloop])) {
        foreach ($programs[$slugloop] as $dzial) {
            if (!in_array($dzial, $addedOptions)) {
                $dzialSelect .= '<option value="' . $dzial . '">' . $dzial . '</option>';
                $addedOptions[] = $dzial; // Add to the array of added options
            }
        }
    }
}
    ...
    // Encode the options array as JSON and send it as the response
    echo json_encode(array('options' => $options, 'dzialSelect' => $dzialSelect));
    wp_die(); 
}

add_action('wp_ajax_list_slug_from_selected_news', 'list_slug_from_selected_news');
add_action('wp_ajax_nopriv_list_slug_from_selected_news', 'list_slug_from_selected_news');